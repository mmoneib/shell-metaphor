...
[ $allocsSum -ne 100 ] && echo "ERROR: The sum of allocated percentages must be equal to 100." && exit 1
# Processing
...
while [ $(( ++count )) -lt $numOfIterations ]; do
  ...
  for (( i=0; i<${#allocatedAmounts[@]}; i++ )); do
    ...
    allocatedAmounts[i]=$(printf %.2f $(echo "scale=2;$amountOfCategory+($deltaFactor*$amountOfCategory*$riskOfCategory)/100"|bc -l))
    allocationsStr+="${catsArr[$i]}=${allocatedAmounts[$i]} -- "
